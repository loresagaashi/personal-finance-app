...
const generateInsights = async (req, res) => {
    ...
    const prevEnd = start;
    const currAgg = await prisma_1.default.transaction.aggregate({ where: { userId, type: 'EXPENSE', date: { gte: start, lt: end } }, _sum: { amount: true } });
    const prevAgg = await prisma_1.default.transaction.aggregate({ where: { userId, type: 'EXPENSE', date: { gte: prevStart, lt: prevEnd } }, _sum: { amount: true } });
