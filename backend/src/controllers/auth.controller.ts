...
export const register = async (req: Request, res: Response) => {
  ...
  res.status(201).json({ user: { id: user.id, email: user.email, name: user.name, isAdmin: user.isAdmin ?? false }, token });
};
...
export const login = async (req: Request, res: Response) => {
  ...
  const token = signJwt({ id: user.id });
  res.json({ user: { id: user.id, email: user.email, name: user.name, isAdmin: user.isAdmin ?? false }, token });
};
